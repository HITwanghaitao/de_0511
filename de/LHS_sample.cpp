...
vector<vector<double>> u_lhs_bounds(int N, int D,vector<vector<double>> bounds) //内部初始抽样，用于拉丁超立方
{
	if (N == 0)
	{
		throw "Sampling number N can't be zero";
...
vector<vector<double>> lhs_bounds(int N, int D, vector<vector<double>> bounds)//拉丁超立方抽样（完整）
{
	vector<vector<double>> r;
	r = u_lhs_bounds(N, D,bounds);
	r = random_sort_N_D(r);
	return r;
}
